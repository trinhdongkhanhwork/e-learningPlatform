...
//                    wallet.setRoleEntity(admin.getRoles().stream().findFirst().orElseThrow(() -> new AppException(ErrorCode.ROLE_NOT_FOUND)));
    ...
    public WalletResponse getWalletByUserId(String userId) {
        ...
        Wallet wallet = walletRespository.findByUser(user)
                .orElseGet(() -> {
                    ...
                    newWallet.setRoleEntity(user.getRoles().stream().findFirst().orElseThrow(() -> new AppException(ErrorCode.ROLE_NOT_FOUND)));
